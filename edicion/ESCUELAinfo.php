...
//
// Table class for ESCUELA
//
class cESCUELA extends cTable {
	var $CLAVE;
	var $CUE;
	var $NOMBRE;
	var $DOMICILIO;
	...
	function __construct() {
		...
		$this->TableVar = 'ESCUELA';
		$this->TableName = 'ESCUELA';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "[ESCUELA]";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// CLAVE
		$this->CLAVE = new cField('ESCUELA', 'ESCUELA', 'x_CLAVE', 'CLAVE', '[CLAVE]', '[CLAVE]', 202, -1, FALSE, '[CLAVE]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->CLAVE->Sortable = TRUE; // Allow sort
		$this->fields['CLAVE'] = &$this->CLAVE;

		// CUE
		$this->CUE = new cField('ESCUELA', 'ESCUELA', 'x_CUE', 'CUE', '[CUE]', '[CUE]', 202, -1, FALSE, '[CUE]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// DOMICILIO
		$this->DOMICILIO = new cField('ESCUELA', 'ESCUELA', 'x_DOMICILIO', 'DOMICILIO', '[DOMICILIO]', '[DOMICILIO]', 202, -1, FALSE, '[DOMICILIO]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// TELEFONO
		$this->TELEFONO = new cField('ESCUELA', 'ESCUELA', 'x_TELEFONO', 'TELEFONO', '[TELEFONO]', '[TELEFONO]', 202, -1, FALSE, '[TELEFONO]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->TELEFONO->Sortable = TRUE; // Allow sort
		$this->fields['TELEFONO'] = &$this->TELEFONO;

		// NIVEL
		$this->NIVEL = new cField('ESCUELA', 'ESCUELA', 'x_NIVEL', 'NIVEL', '[NIVEL]', '[NIVEL]', 3, -1, FALSE, '[NIVEL]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->NIVEL->Sortable = TRUE; // Allow sort
		$this->NIVEL->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->NIVEL->PleaseSelectText = $Language->Phrase("PleaseSelect"); // PleaseSelect text
		$this->NIVEL->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['NIVEL'] = &$this->NIVEL;

		// RPV
		$this->RPV = new cField('ESCUELA', 'ESCUELA', 'x_RPV', 'RPV', '[RPV]', '[RPV]', 202, -1, FALSE, '[RPV]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->RPV->Sortable = TRUE; // Allow sort
		$this->fields['RPV'] = &$this->RPV;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "[ESCUELA]";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "ESCUELAedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "ESCUELAadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("ESCUELAview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("ESCUELAview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "ESCUELAadd.php?" . $this->UrlParm($parm);
		else
			$url = "ESCUELAadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("ESCUELAedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("ESCUELAadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("ESCUELAdelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->RPV->setDbValue($rs->fields('RPV'));
	}
	...
	function RenderEditRow() {
		...
		$this->RPV->EditValue = $this->RPV->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->RPV->Exportable) $Doc->ExportCaption($this->RPV);
				} else {
					if ($this->CLAVE->Exportable) $Doc->ExportCaption($this->CLAVE);
					if ($this->CUE->Exportable) $Doc->ExportCaption($this->CUE);
					if ($this->DOMICILIO->Exportable) $Doc->ExportCaption($this->DOMICILIO);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->RPV->Exportable) $Doc->ExportField($this->RPV);
					} else {
						if ($this->CLAVE->Exportable) $Doc->ExportField($this->CLAVE);
						if ($this->CUE->Exportable) $Doc->ExportField($this->CUE);
						if ($this->DOMICILIO->Exportable) $Doc->ExportField($this->DOMICILIO);
